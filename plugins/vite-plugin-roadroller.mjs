  ...
  return (tree) => {
    ...
    function handleNodes (node) {
      ...
      const asset = assets.find((asset) =>
        node.attrs?.href?.endsWith(asset.fileName))
      if (asset != null) {
...
async function embedCSS (src, cssAssets) {
  const html = (await posthtml().use(inlineStyleTag(cssAssets)).process(src))
    .html
...
async function embedJS (src, jsAssets) {
  const html = (await posthtml().use(removeScriptTag(jsAssets)).process(src))
    .html
    ...
    transformIndexHtml: {
      ...
      handler: async (src, id) => {
        ...
        const remainingAssets = assets.filter(
          (asset) =>
            !asset.fileName.endsWith('.css') && !asset.fileName.endsWith('.js')
        )
